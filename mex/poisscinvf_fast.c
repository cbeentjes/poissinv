 ...
 * poisscinvf_fast.c - 
 *
 * Computes the inverse Poisson complementary CDF efficiently in single precision
 *
 * The calling syntax is:
 *
 *		outMatrix = poisscinvf_fast(U, lambda)
 *
...
void poisscinvf_fast_full(float *U, float *lam, float *Z, mwSize n);
void poisscinvf_fast_l(float *U, float *lam, float *Z, mwSize n);
void poisscinvf_fast_U(float *U, float *lam, float *Z, mwSize n);
                 ...
                 
{
    ...
    if (nrhs!=2) {
        mexErrMsgIdAndTxt("MyToolbox:poisscinvf_fast:nrhs","Two input arguments required.\n");
    }
    if (nlhs>1) {
        mexErrMsgIdAndTxt("MyToolbox:poisscinvf_fast:nlhs","Too many output arguments.\n");
    }

    /* make sure the input arguments are type float */
    if (!mxIsSingle(prhs[0]) 
        || mxIsComplex(prhs[0])) {
        mexErrMsgIdAndTxt("MyToolbox:poisscinvf_fast:notDouble","Input P must be real of type float.\n");
    }
    if (!mxIsSingle(prhs[1])  
        || mxIsComplex(prhs[1])) {
        mexErrMsgIdAndTxt("MyToolbox:poisscinvf_fast:notDouble","Input lambda must be real of type float.\n");
    }
    ...
    if (mxGetNumberOfDimensions(prhs[0])!=mxGetNumberOfDimensions(prhs[1])
        ...
        mexErrMsgIdAndTxt("MyToolbox:poisscinvf_fast:InputSizeMismatch","Requires non-scalar arguments to match in size.\n");
    }
        ...
        && mxGetNumberOfElements(prhs[1])!=1) 
    {
        for (mwSize d=0; d<mxGetNumberOfDimensions(prhs[0]); d++) {
            if (dims_lambda[d]!=dims_U[d]) {
                mexErrMsgIdAndTxt("MyToolbox:poisscinvf_fast:InputSizeMismatch","Inputs must have the same dimensions.\n");
            }
        }
    }
    ...
    if (mxGetNumberOfElements(prhs[0])==1) {
       poisscinvf_fast_U(U_matrix, lambda_matrix, out_matrix, n_out);
    } else if (mxGetNumberOfElements(prhs[1])==1) {
       poisscinvf_fast_l(U_matrix, lambda_matrix, out_matrix, n_out);
    } else {
       poisscinvf_fast_full(U_matrix, lambda_matrix, out_matrix, n_out);
    }
}
