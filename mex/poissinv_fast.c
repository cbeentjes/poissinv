 ...
 * poissinv_fast.c - 
 *
 * Computes the inverse Poisson CDF efficiently
 *
 * The calling syntax is:
 *
 *		outMatrix = poissinv_fast(U, lambda)
 *
...
void poissinv_fast_full(double *U, double *lam, double *Z, mwSize n);
void poissinv_fast_l(double *U, double *lam, double *Z, mwSize n);
void poissinv_fast_U(double *U, double *lam, double *Z, mwSize n);
                 ...
                 
{
    ...
    if (nrhs!=2) {
        mexErrMsgIdAndTxt("MyToolbox:poissinv_fast:nrhs","Two input arguments required.\n");
    }
    if (nlhs>1) {
        mexErrMsgIdAndTxt("MyToolbox:poissinv_fast:nlhs","Too many output arguments.\n");
    }

    /* make sure the input arguments are type double */
    if (!mxIsDouble(prhs[0]) 
        || mxIsComplex(prhs[0])) {
        mexErrMsgIdAndTxt("MyToolbox:poissinv_fast:notDouble","Input P must be real of type double.\n");
    }
    if (!mxIsDouble(prhs[1])  
        || mxIsComplex(prhs[1])) {
        mexErrMsgIdAndTxt("MyToolbox:poissinv_fast:notDouble","Input lambda must be real of type double.\n");
    }
    ...
    if (mxGetNumberOfDimensions(prhs[0])!=mxGetNumberOfDimensions(prhs[1])
        ...
        mexErrMsgIdAndTxt("MyToolbox:poissinv_fast:InputSizeMismatch","Requires non-scalar arguments to match in size.\n");
    }
        ...
        && mxGetNumberOfElements(prhs[1])!=1) 
    {
        for (mwSize d=0; d<mxGetNumberOfDimensions(prhs[0]); d++) {
            if (dims_lambda[d]!=dims_U[d]) {
                mexErrMsgIdAndTxt("MyToolbox:poissinv_fast:InputSizeMismatch","Inputs must have the same dimensions.\n");
            }
        }
    }
    ...
    if (mxGetNumberOfElements(prhs[0])==1) {
       poissinv_fast_U(U_matrix, lambda_matrix, out_matrix, n_out);
    } else if (mxGetNumberOfElements(prhs[1])==1) {
       poissinv_fast_l(U_matrix, lambda_matrix, out_matrix, n_out);
    } else {
       poissinv_fast_full(U_matrix, lambda_matrix, out_matrix, n_out);
    }
}
